...
동작한다. 중앙 PC는 Domain 200에서 두 차량의 상태를 Foxglove로 관측하고,
...
| 중앙 PC | 200 | Domain Bridge, Foxglove Bridge, 관제 명령 발행 |
...
중앙 PC의 `domain_bridge` 한 프로세스가 Domain 1, 2, 200의 참가자를 생성한다.
토픽별 `from_domain`과 `to_domain`을 명시해 자동 브로드캐스트나 와일드카드를
사용하지 않는다.

```text
robot_1 Domain 1  -- 차량 상태 -->\
                                domain_bridge --> 중앙 Domain 200 --> Foxglove
robot_2 Domain 2  -- 차량 상태 -->/

중앙 Domain 200 -- robot_1 namespace 명령 --> Domain 1
중앙 Domain 200 -- robot_2 namespace 명령 --> Domain 2
```
...
각 차량에 다음 이름공간 토픽을 Domain 200으로 복제한다.
...
| 중앙 Domain 200 토픽 | 목적 차량 Domain 토픽 | 타입 | 경로 |
| --- | --- | --- | --- |
...
항목은 Domain 2만 목적지로 갖는다. 공용 `/cmd_vel` 또는 이름공간 없는 goal
토픽은 만들지 않는다.
...
2. `foxglove-bridge`: Domain 200에서만 실행하며 Domain Bridge가 제공한 토픽을
   WebSocket으로 노출한다.

`central-up`, `central-down`, `central-logs`, `central-topics` 명령을 추가한다.
`sim-up`, mapping, navigation 명령과 기존 Foxglove 포트 계약은 변경하지 않는다.
...
- 중앙 Domain 200 용 Docker Compose, 환경 예시, 브리지 YAML
...
1. Compose 정적 검사에서 중앙 서비스의 `ROS_DOMAIN_ID=200`과 host network를
   확인한다.
...
4. 시뮬레이션 통합 테스트에서 Domain 1과 2의 대체 publisher를 사용해 중앙
   Domain 200에서 두 차량 관측 토픽을 받고, 각 중앙 명령이 정확히 하나의
   차량 Domain으로만 전달됨을 확인한다.

...
if [[ "$mode" == 'localization' ]]; then
  printf 'mentorpi navigation_mode=localization session_id=%s map_yaml=%s\n' \
    "$session_id" "$map_yaml"
  exec ros2 launch mentorpi_nav navigation.launch.py \
    mode:=localization map_yaml:="$map_yaml"
fi

...
- `transport_operations`: 비적재 차량은 `TO_PICK` 상태로 source zone을, 적재 차량은 `TO_PLACE` 상태로 destination zone을 다음 주행 목적지로 받습니다.

PICK 완료는 source 재고를 1 감소시키고 차량 상태를 적재로 바꿉니다. PLACE 완료는 destination 재고를 1 증가시키고 차량 상태를 비적재로 바꿉니다. 각 전이는 단일 SQLite transaction으로 처리하며 `idempotency_key`가 같은 완료 보고는 같은 이벤트를 반환합니다.

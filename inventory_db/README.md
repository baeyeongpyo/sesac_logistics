...
- `transport_operations`: 비적재 차량은 `TO_PICK` 상태로 source zone을, 적재 차량은 `TO_PLACE` 상태로 destination zone을 다음 주행 목적지로 받습니다. 완료·취소되지 않은 작업은 destination의 적치 슬롯 1개도 예약합니다.

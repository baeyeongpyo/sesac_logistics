...
def detect_nearest_topline(frame, anchor_px=None):
    return inspect_nearest_topline(frame,anchor_px)['detection']


def inspect_nearest_topline(frame, anchor_px=None):
    ...
    rejected=[]
    for identity in sorted(xs, key=lambda d: np.linalg.norm(np.asarray(d['center_px'])-centre)):
        x, y, w, h = identity['roi_px']
        ...
        if band is not None:
            detection=side_intersections(frame, band)
...
def detect_locked_topline(frame, reference_frame, reference):
    ...
    return dict(band,top_edge_px=[s['point'] for s in sides],side_bands=sides,

...
# Import only the existing pure numerical planner; never auto_dock_node.
try:
    ...
    sys.path.insert(0, str(Path(__file__).resolve().parents[1]/'ros2_ws_src'/'auto_dock'))
    from auto_dock.loaded_response_planner import (
        ResponseCoefficients, ResponseState, plan_approach, simulate_actions, advance_state,
    )
...
def measured_response(command_rows, imu_rows, coefficients):
    ...
    for i,(a,b) in enumerate(zip(edges[:-1],edges[1:])):
        ...
        # A startup pulse does not identify sustained turn response. Retain
        # calibrated immediate gain, while still estimating released total yaw.
        if b[0]-a[0]<.5:
            immediate=coefficients.yaw_gains(a[3])[0]
    ...
    for direction in samples:
        ...
        evidence[direction] = dict(samples=len(values), source='approach_imu' if values else 'saved_model',
                                  short_pulses_using_saved_immediate=short_samples[direction])
        if values:

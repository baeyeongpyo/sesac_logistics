...
bool ControllerServer::isGoalReached()
{
  ...
  if (!getRobotPose(pose)) {
    RCLCPP_WARN(get_logger(), "[SERVER GOAL CHECK] Failed to get robot pose");
    return false;
  }
  ...
  // Use the same timestamp as the current robot pose when transforming the goal.
  // This prevents comparing the current robot pose against a goal transformed
  // using an older map->odom transform.
  geometry_msgs::msg::PoseStamped goal_pose = end_pose_;
  goal_pose.header.stamp = pose.header.stamp;

  nav_2d_utils::transformPose(
    costmap_ros_->getTfBuffer(), costmap_ros_->getGlobalFrameID(),
    goal_pose, transformed_end_pose, tolerance);

  const double robot_x = pose.pose.position.x;
  const double robot_y = pose.pose.position.y;

  const double goal_x = transformed_end_pose.pose.position.x;
  const double goal_y = transformed_end_pose.pose.position.y;

  const double distance =
    std::hypot(goal_x - robot_x, goal_y - robot_y);

  double xy_tolerance = -1.0;
  double yaw_tolerance = -1.0;

  get_parameter(
    current_goal_checker_ + ".xy_goal_tolerance",
    xy_tolerance);

  get_parameter(
    current_goal_checker_ + ".yaw_goal_tolerance",
    yaw_tolerance);

  const bool result =
    goal_checkers_[current_goal_checker_]->isGoalReached(
      pose.pose,
  ...
  RCLCPP_WARN(
    get_logger(),
    "[SERVER GOAL CHECK] "
    "robot_frame=%s end_frame=%s transformed_goal_frame=%s global_frame=%s "
    "robot_stamp=%d.%09u end_stamp=%d.%09u transformed_goal_stamp=%d.%09u "
    "robot=(%.4f, %.4f) "
    "goal=(%.4f, %.4f) "
    "distance=%.4f "
    "xy_tol=%.4f "
    ...
    transformed_end_pose.header.frame_id.c_str(),
    ...
    transformed_end_pose.header.stamp.sec,
    transformed_end_pose.header.stamp.nanosec,
    robot_x,
    robot_y,
    goal_x,
    goal_y,
